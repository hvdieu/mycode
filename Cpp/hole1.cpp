...
inline bool check(int i, int j, int len1, int len2) {
    int x, u = i - len1 + 1, v = j - len2 + 1;
    x = s1[i][j] - s1[u-1][j];
    if (x) return false;
...
void solve() {
    ...
    For(i, 2, n-1) {
        ...
        if (j > 1) {
                ...
                int d1, c1, d2, c2;
                if (h[j] < g[j]) {
                    For(len1, 1, h[j]) Rep(len2, g[j], res / len1 + 1)
                    if (check(i, j, len1, len2)) {
                        res = max(res, len1 * len2);
                        break;
                    }
                } else {
                    For(len2, 1, g[j]) Rep(len1, h[j], res / len2 + 1)
                    if (check(i, j, len1, len2)) {
                        res = max(res, len1 * len2);
                        break;
                    }
...
int main() {
    freopen("hole1.inp","r",stdin);
    freopen("hole1.out","w",stdout);
    scanf("%d", &n);
