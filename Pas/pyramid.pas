...
Var     fi,fo   :       text;
        s,sab,scd,mincd       :       array [0..maxn,0..maxn] of longint;
        ...
        begin
             ...
             for i := c to m do // tim mincd theo hang
                begin
                    ...
                    for j := d to d+k-2 do
                     begin
                          while (top >= bot) and (scd[i,j] < st[top]) do dec(top);
                          inc(top);
                          st[top] := scd[i,j];
                          p[top] := j;
                     end;
                    for j := d to n - k + 1 do
                     begin
                          while (top >= bot) and (scd[i,j+k-1] < st[top]) do dec(top);
                          inc(top);
                          st[top] := scd[i,j+k-1]; p[top] := j+k-1;
